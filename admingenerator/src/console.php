...
$console = new Application('POS ADMIN', '1.0');
...
$console
	...
	->setCode(function (InputInterface $input, OutputInterface $output) use ($app) {
		...
		//$getTablesResult = $app['db']->fetchAll($getTablesQuery, array());
                $getTablesResult = array('catalogo');

		$_dbTables = array();
		$dbTables = array();
    
//var_dump($getTablesResult);
...
///// JOMR ini
                                        if ($table_name=="adicionables" && $column['Field']=="ID_grupo"){
                                            $_table_name ="productos_grupos";
                                            $external_table=$_table_name;
                                        }else if ($table_name=="adicionables_producto" && $column['Field']=="ID_adicional" ){
                                            $_table_name ="adicionables";
                                            $external_table=$_table_name;
                                        }else if ($table_name=="adicionables_producto" &&  $column['Field']=="ID_producto"){
                                            $_table_name ="productos";
                                            $external_table=$_table_name;
                                        }else if ($table_name=="productos" &&  $column['Field']=="ID_grupo"){
                                            $_table_name ="productos_grupos";
                                            $external_table=$_table_name;
                                        }else if ($table_name=="productos_ingredientes" &&  $column['Field']=="ID_producto"){
                                            $_table_name ="productos";
                                            $external_table=$_table_name;
///// JOMR end
                                       
                                        }else if(substr($column['Field'], -3) == "_id" ){
...
//// JOMR ini
                                        ...
                                        if ($external_table=="adicionables" ){
                                            
                                            $external_select_field="nombre";
                                        }else if ($external_table=="productos" ){
                                            
                                            $external_select_field="nombre";
                                        }else if ($external_table=="productos_grupos" ){
                                            
                                            $external_select_field="descripcion";        
                                        }else{
                                        /*
                                        else if ($external_table=="adicionables_producto" ){
                                            //&& $column['Field']=="ID_adicional" 
                                            $external_select_field="nombre";
                                        }else if ($external_table=="adicionables_producto" ){
                                            //&&  $column['Field']=="ID_producto"
                                            $external_select_field="nombre";
                                        }else if ($external_table=="productos_ingredientes" ){
                                            //&&  $column['Field']=="ID_producto"
                                            $external_select_field="nombre";        
                                        }
                                         */
///// JOMR end
					...
					$search_names_foreigner_key = array('nombre','descripcion','usuario');
