...
function actualizarTotales() {
    ...
    rsv_solicitar('cortez',{fecha: fecha.toString('yyyy-MM-dd')},function(datos){
        ...
        buffer += '<p>Total Prods. eliminados: $' + datos.aux.total_cancelado + '</p>';
        buffer += '<p>Total descuentos: $' + datos.aux.total_descuentos + '</p>';
        buffer += '<p>Total compras: $' + datos.aux.total_compras + '</p>';
        buffer += '<p>Total comprasG: $' + datos.aux.total_comprasG + '</p>';
        buffer += '<p>Total comprasO: $' + datos.aux.total_comprasO + '</p>';
                    ...
                    '<th>Total</th>' +
                            '<th>Diferencia</th>' +
                                    '<th>Efectivo</th>' +
                                            ...
                                            '<th>Compras</th>' +
                                             '<th>ComprasG</th>' +
                                            '<th>Caja</th>'+ // + '<th>Estado</th></tr>';
                                            '<th>CompraO</th>';
        for(y in datos.aux.historial)
        {
            buffer += '<tr>' + '<td>'+ datos.aux.historial[y].fechatiempo + '</td>' + 
...
//                    + '<td><select id="sel_estado_corte_' + datos.aux.historial[y].ID_cortes + 
//                    '"><option value="pendiente">Pendiente</option><option ' + 
//                    (datos.aux.historial[y].estado == 'recibido' ? 'selected="selected"' : '') + 
//                    ' value="recibido">Recibido</option><option ' + 
//                    (datos.aux.historial[y].estado == 'remesado' ? 'selected="selected"' : '') + 
//                    ' value="remesado">Remesado</option></select>'
//                    + '<button class="guardar_estado_corte" rel="' + 
//                    datos.aux.historial[y].ID_cortes + '">guardar</button></td>' + 
                    '</tr>' ;
        }
        buffer += '</table>';
...
function actualizarCortes() {
    $('#historial_cortez').click(function(){
        rsv_solicitar('cortez',{historial: true},function(datos){
            var buffer = '<table class="estandar ancha bordes">';
            buffer += '<tr><th>Fecha</th><th>Total</th><th>Diferencia</th><th>Efectivo</th>'+
                    '<th>POS</th><th>Compras</th><th>Caja</th><th>Estado</th></tr>';
            for(y in datos.aux.historial)
            {
                buffer += '<tr>' + '<td>'+ datos.aux.historial[y].fechatiempo + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_a_cuadrar + '</td>' + 
                        ...
                        '<td>'+ datos.aux.historial[y].total_compras + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_caja + '</td>' + 
                        '<td>'+ datos.aux.historial[y].estado + '</td>' + 
                        '</tr>' ;
            }
            buffer += '</table>';
