...
//        }
    ...
    function adminPass(data){
        ...
        meseroPassword="";                                                
        meseroPassword = window.prompt('Ingrese su clave de Admin: ');
        if (!meseroPassword){
            return d1.resolve({status:false,data:data});
        }            
            
        consulta="select ID_usuarios from usuarios where disponible=1 AND nivel='Gerente' AND clave = sha1('" + meseroPassword + "')";                
        
        rsv_solicitar('consultarUtils', {consulta:consulta, consultaType:"1"}, function(datos){                        
            
            if (datos.aux !==""){                
                return d1.resolve({status:true,data:data});
            }else{
                alert('Usuario no autorizado. Ingresar de nuevo ' );
                adminPass(data);
            }
        }, false); 
