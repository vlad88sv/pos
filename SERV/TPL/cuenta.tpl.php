...
$c = 'SELECT (SELECT nota FROM historial st0 ' .
        'WHERE ID_pedido>0 AND st0.ID_pedido = t1.ID_pedido ORDER BY ID_historial DESC LIMIT 1) AS "historia", ' .
        't0.ID_mesero, t4.usuario AS "nombre_mesero", t0.`ID_domicilio`, t1.`ID_cuenta`, t1.`fechahora_despachado`, ' .
        't1.`fechahora_pedido` , unix_timestamp(t1.`fechahora_pedido`) AS "fechahora_pedido_uts" , ' .
        't1.`fechahora_despachado` , t0.`fechahora_pagado` , t0.`flag_nopropina` , t0.`flag_exento` , ' .
        't0.`flag_pagado`, t1.`flag_elaborado` , t1.`flag_despachado` ,  t0.`flag_anulado`, t0.`flag_tiquetado`, ' .
        't1.`flag_cancelado`, t0.`metodo_pago`, t0.`ID_mesa` , t0.`ID_usuario` , t1.`ID_pedido` , t1.`ID_producto` , ' .
        't1.`precio_grabado` , t2.`nombre` AS "nombre_producto", t1.`tmpID`, t1.`flag_cancelado`, t2.ID_grupo, ' .
        't3.descripcion AS "grupo_desc", t0.ID_comanda
            FROM `pedidos` AS t1
...
{
    $c = 'SELECT t1.ID_pedido_adicional, t2.nombre, t1.precio_grabado, t1.tipo '
            . 'FROM `pedidos_adicionales` AS t1 LEFT JOIN `adicionables` AS t2 USING(ID_adicional) '
            . 'WHERE ID_pedido="'.$f['ID_pedido'].'" AND tipo="poner"';
